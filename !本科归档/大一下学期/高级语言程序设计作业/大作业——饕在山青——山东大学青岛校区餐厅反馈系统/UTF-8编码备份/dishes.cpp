...
void OutPutDishes()
{
	...
	for (int i = 0; i != dishes_names.size(); i++)
	{
		...
		{
			tem_file << "炒菜类 ";
			tem_file << tem_name << " "<<fired_dishs.at(tem_name).GetIngredients()<<" "<<fired_dishs.at(tem_name).GetPlace()<<" "<<fired_dishs.at(tem_name).GetPrice()<<" "<<fired_dishs.at(tem_name).GetExtraOne()<<" "<<fired_dishs.at(tem_name).GetExtraTwo()<<"\n";
		}
...
void OutPutComments()
{
	...
	for (auto i = fired_dishs.begin(); i != fired_dishs.end(); i++)
	{
		if (!(*i).second.GetComments().empty())
		{
			...
			{
				tem_file << (*i).first << " " << "反馈：" + (*i).second.this_comments[j] << "\n";
			}
		}
	}
