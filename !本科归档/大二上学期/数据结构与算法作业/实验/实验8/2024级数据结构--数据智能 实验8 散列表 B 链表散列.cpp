/*2024级数据结构--数据智能 实验8 散列表 B 链表散列.cpp*/
#include<iostream>
...
private:
    ...
    int capacity;
public:
    chainHashTable(const T& ini_divisor);
    chainHashTable();
...
template<class T>
chainHashTable<T>::chainHashTable(const T& ini_divisor)
{
    this->divisor = ini_divisor;
    this->capacity = ini_divisor;
    data = new chainlist<T>[capacity];
}

template<class T>
void chainHashTable<T>::insert(const T& key)
{
    int index = key % divisor;
    if (data[index].find_first(key) != -1)
    {
        cout << "Existed" << endl;
    }
    else
...
template<class T>
void chainHashTable<T>::find(const T& key)
{
    int index = key % divisor;
    if (data[index].find_first(key) == -1)
    {
    ...
    {
        cout << data[index].getSize() << endl;
    }
}

template<class T>
void chainHashTable<T>::erase(const T& key)
{
    int index = key % divisor;
    if (data[index].find_first(key) == -1)
    {
    ...
    {
        data[index].erase(data[index].find_first(key));
        cout << data[index].getSize() << endl;
    }
}
...
void Solution::solve()
{
    int n, m;
    cin >> n >> m;
    chainHashTable<int> hashTable(n);
    for (int i = 0; i < m; i++)
    {
        int a, b;
        cin >> a >> b;
