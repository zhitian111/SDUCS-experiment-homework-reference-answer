/*2024级数据结构--数据智能 实验9 二叉树 B 二叉树遍历.cpp*/
#include <iostream>
...
{
public:
    enum inputType { preOrder, inOrder, postOrder, minxOrder };
private:
    Node<T>* root;
    size_t count;
    size_t depth;
...
public:
    ...
    void buildTree(Node<T>* root,T preOrder[], T inOrder[], size_t size,bool direction = true);
};
template<class T>
...
binaryTree<T>::binaryTree(size_t size, binaryTree<T>::inputType type)
{
    ...
    else if (type == binaryTree<T>::inputType::minxOrder)
    {
        T* preOrder = new T[size];
        T* inOrder = new T[size];
        for (size_t i = 0; i < size; i++)
        {
            cin >> preOrder[i];
        }
        ...
        this -> root = new Node<T>(nullptr, preOrder[0]);


        buildTree(this->root, preOrder, inOrder, size);
...
template<class T>
void binaryTree<T>::buildTree(Node<T>* root, T preOrder[], T inOrder[], size_t size , bool direction)
{
    if (root == nullptr)
    {
        return;
    ...
    if (size == 1)
    {
        // if (direction)
        // {
        //     root->left = new Node<T>(root, preOrder[0]);            
        // }
        // if (!direction)
        // {
        //     root->right = new Node<T>(root, preOrder[0]);
        // }
        return;
    }
    size_t rootIndex = 0;
    ...
    size_t leftSize = rootIndex;
    size_t rightSize = size - leftSize - 1;

    T* leftPreOrderHead = &preOrder[1];
    
    T* rightPreOrderHead = &preOrder[rootIndex + 1];

    T* leftInOrderHead = &inOrder[0];

    T* rightInOrderHead = &inOrder[rootIndex + 1];


    if (leftSize > 0)
    {
        root->left = new Node<T>(root, preOrder[1]);
        buildTree(root->left, leftPreOrderHead, leftInOrderHead, leftSize, true);        
    }
    if (rightSize > 0)
    {
        root->right = new Node<T>(root, preOrder[rootIndex + 1]);
        buildTree(root->right, rightPreOrderHead, rightInOrderHead, rightSize, false);        
    }
}
...
void Solution::solve()
{
    ...
    binaryTree<int> tree(size, binaryTree<int>::inputType::minxOrder);
