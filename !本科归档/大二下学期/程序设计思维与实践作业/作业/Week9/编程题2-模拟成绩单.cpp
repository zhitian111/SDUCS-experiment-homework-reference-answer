...
template <typename T, bool Directed, bool Weighted, typename weight_type>
vector<T> Graph<T, Directed, Weighted, weight_type>::topological_sort() const
{
    vector<T> res;
    vector<size_t> in_degree(nodes.size(), 0);
    ...
    while (!q.empty())
    {
        auto node = q.top();
        q.pop();
        res.push_back(nodes[node].data);
        for (auto& edge : nodes[node].edges)
        {
            in_degree[edge.to - 1]--;
            if (in_degree[edge.to - 1] == 0)
            {
                q.push(edge.to - 1);
            }
        }
