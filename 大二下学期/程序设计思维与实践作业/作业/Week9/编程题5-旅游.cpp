...
void solve()
{
    ...
    double values[n + 1][n + 1], prices[n + 1][n + 1];
    for (int i = 0; i < m; i++)
    {
        int u, v;
        double value, price;
    ...
    while (left + 1e-6 < right)
    {
        ...
        for (auto& node : g.nodes)
        {
            for (auto& edge : node.edges)
            {
                edge.weight = mid * prices[edge.from][edge.to] - values[edge.from][edge.to];
                // cout << "price " << prices[edge.from][edge.to] << " value " << values[edge.from][edge.to] << " weight " << edge.weight << endl;
            }
        }
        if (g.isExistInverseRing())
        {
            left = mid;
        }
        else
